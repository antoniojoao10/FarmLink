...
public class MyAdapterProfile extends RecyclerView.Adapter<MyAdapterProfile.MyViewHolder> implements Filterable {

    Context context;
    private OnFruitListener monFruitListener;

    public MyAdapterProfile(Context ct, OnFruitListener onFruitListener){
        context = ct;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        holder.myText1.setText(Variables.productNames.get(Variables.order.get(position)));
        holder.myText3.setText(Variables.productQuantaty.get(Variables.order.get(position))+"Kg");

        StorageReference storageReference = FirebaseStorage.getInstance().getReference();

        StorageReference photoReference= storageReference.child("fruitsImages/" + Variables.imagesNames.get(Variables.order.get(position)) + ".jpg");
