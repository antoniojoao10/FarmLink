...
public class MyAdapterBought extends RecyclerView.Adapter<MyAdapterBought.MyViewHolder> implements Filterable {

    Context context;
    private OnFruitListener monFruitListener;

    public MyAdapterBought(Context ct, OnFruitListener onFruitListener){
        context = ct;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        String[] tmp = Variables.names.get(position).split("\\+");

        holder.myText1.setText(tmp[1]);
        holder.myText3.setText(Variables.productBought.get(Variables.names.get(position))+"Kg");

        StorageReference storageReference = FirebaseStorage.getInstance().getReference();

        StorageReference photoReference= storageReference.child("fruitsImages/" + Variables.imagesNames.get(Variables.productNames.indexOf(tmp[1])) + ".jpg");
