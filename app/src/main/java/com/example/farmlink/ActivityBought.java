...
public class ActivityBought extends AppCompatActivity {
    private RecyclerView recycle;
    public MyAdapterBought myAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (String i : Variables.productBought.keySet()) {
            String[] tmp = i.split("\\+");
            prep.add(Integer.parseInt(tmp[0]));
        }
        Collections.sort(prep);
        for (int i = 0; i < prep.size() ; i++ ){
            for (String a : Variables.productBought.keySet()) {
                String[] tmp = a.split("\\+");
                if(tmp[0].equals(String.valueOf(prep.get(i)))) Variables.names.add(a);
            }
        }
        ...
        recycle = findViewById(R.id.recyclerBought);


        myAdapter = new MyAdapterBought(getApplicationContext(), this::onFruitClick);
        recycle.setAdapter(myAdapter);
        recycle.setLayoutManager(new LinearLayoutManager(getApplicationContext()));

        swipeRefreshLayout= findViewById(R.id.refreshBought);
